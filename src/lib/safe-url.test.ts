...
describe("isAllowedExternalUrl — allowed schemes", () => {
  it("accepts https URLs", () => {
    expect(
      isAllowedExternalUrl(
        "https://github.com/tburkhalterr/CANShift/releases/tag/v1.0.0",
      ),
    ).toBe(true);
  });
  ...
  it("accepts mailto with a body query", () => {
    expect(
      isAllowedExternalUrl("mailto:hello@example.com?subject=hi&body=ok"),
    ).toBe(true);
  });
});
...
describe("isAllowedExternalUrl — blocked schemes", () => {
  ...
  it("rejects intent: (Android Intent resolution)", () => {
    expect(
      isAllowedExternalUrl(
        "intent://path#Intent;scheme=mal;package=com.evil;end",
      ),
    ).toBe(false);
  });
  ...
  it("rejects data: (inline payload)", () => {
    expect(isAllowedExternalUrl("data:text/html,<script>1</script>")).toBe(
      false,
    );
  });
